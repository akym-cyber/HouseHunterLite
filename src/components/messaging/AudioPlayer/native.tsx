...
const AudioPlayerNative: React.FC<AudioPlayerProps> = ({
  remoteUrl,
  duration,
  waveform,
  isOwnMessage,
  ...
  const isSupportedFormat = resolvedFormat !== 'webm';
  ...
  const debugRemote = remoteUrl
    ? `${remoteUrl.slice(0, 42)}...${remoteUrl.slice(-12)}`
    : 'none';
  ...
  const onPlaybackStatusUpdate = useCallback((status: Audio.SoundStatus) => {
    if (!status.isLoaded) {
      if ('error' in status && status.error) {
        console.warn('[AudioPlayerNative] Playback error:', status.error);
  ...
  const resolveAudioUri = useCallback(async (sourceUrl: string): Promise<string> => {
    ...
    try {
      ...
      console.warn('[AudioPlayerNative] Cache download failed, using remote URL:', error);
      return sourceUrl;
    }
  }, []);
  ...
  const loadSound = useCallback(async () => {
    ...
    try {
      ...
      if (!safeUrl) {
        console.warn('[AudioPlayerNative] Missing audio URL');
        return null;
      }
      ...
      console.warn('[AudioPlayerNative] Failed to load audio:', error);
  ...
  const togglePlayback = useCallback(async () => {
    try {
      ...
      console.warn('[AudioPlayerNative] Toggle playback failed:', error);
...
export default AudioPlayerNative;
